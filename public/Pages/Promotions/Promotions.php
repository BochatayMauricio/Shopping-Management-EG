...
// Definimos la fecha de hoy una sola vez
$today = date('Y-m-d');
...
// Definición de jerarquía: Premium (3) > Medium (2) > Inicial (1)
$levelWeights = [
    'inicial' => 1,
    'medium'  => 2,
    'premium' => 3
...
$userCategory = isset($user['category']) ? strtolower($user['category']) : 'inicial';
$userWeight = $levelWeights[$userCategory] ?? 1;
...
<?php foreach ($filteredPromotions as $promo): ?>
    <?php 
        ...
        // NUEVA LÓGICA: Determinar si la promoción está vencida
        // Compara la fecha de la promo con la variable $today definida al inicio del archivo
        $isExpired = ($promo['date_until'] < $today);
    ?>
    
    <div class="promo-card <?php echo $isLocked ? 'promo-locked' : ''; ?> <?php echo $isExpired ? 'promo-expired' : ''; ?>">
        <div class="promo-image-container">
            ...
            <?php if ($isExpired): ?>
                <div class="promo-lock-overlay" style="background: rgba(0,0,0,0.6);">
        ...
        <div class="promo-content">
            ...
            <p class="promo-validity <?php echo $isExpired ? 'text-danger fw-bold' : ''; ?>">
                Válido hasta: <?php echo htmlspecialchars($promo['valid_until']); ?>
                <?php echo $isExpired ? ' (Vencida)' : ''; ?>
            </p>
            ...
            <?php if ($user && $user['type'] === 'client'): ?>
                ...
                <form action="" method="POST">
                    ...
                    <?php if ($isExpired): ?>
                        <button type="button" class="promo-request-btn" style="background-color: #343a40; border-color: #343a40;" disabled>
                            <i class="fas fa-times-circle me-2"></i> No disponible
                        </button>

                    <?php elseif ($requestStatus === 'active'): ?>
                        <button type="button" class="promo-request-btn btn-obtained" disabled>
                            <i class="fas fa-check-circle me-2"></i> Promoción Obtenida
                        </button>
                        <small class="text-success d-block text-center mt-2 fw-bold">¡Ya está en tu portal!</small>

                    <?php elseif ($requestStatus === 'used'): ?>
                        <button type="button" class="promo-request-btn btn-used" disabled>
                            <i class="fas fa-history me-2"></i> Ya canjeada
                        </button>

                    <?php elseif ($isLocked): ?>
                        <button type="submit" name="btnRequestPromo" class="promo-request-btn btn-locked" disabled>
                            <i class="fas fa-lock me-2"></i> Nivel <?php echo htmlspecialchars($promo['client_category']); ?>
                        </button>

                    <?php else: ?>
                        <button type="submit" name="btnRequestPromo" class="promo-request-btn">
                            <i class="fas fa-ticket-alt me-2"></i> Solicitar Promoción
                        </button>
                    <?php endif; ?>
                </form>
            <?php endif; ?>
